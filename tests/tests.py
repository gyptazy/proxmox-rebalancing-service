...
from proxmox_rebalancing_service import (
    initialize_logger,
    pre_validations,
    post_validations,
    validate_daemon,
...
class TestProxmoxRebalancingService(unittest.TestCase):
    ...
    def test_pre_validations(self):
        with patch('proxmox_rebalancing_service.__validate_imports') as mock_validate_imports, patch('proxmox_rebalancing_service.__validate_config_file') as mock_validate_config_file:
    ...
    def test_initialize_config_path(self):
        ...
        with patch('logging.info') as mock_info:
            config_path = initialize_config_path(app_args)
            self.assertEqual(config_path, '/etc/proxmox-rebalancing-service/prs.conf')
            self.assertTrue(mock_info.called)
